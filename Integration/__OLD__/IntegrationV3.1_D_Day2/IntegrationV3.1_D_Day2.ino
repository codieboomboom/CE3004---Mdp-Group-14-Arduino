 ...
 * 1.Less strict logic for front calibration
 * Calibration on if 1 block is detected
 * 
 * 2.Retune final PID for going straight, turning right, turning left
 * Experment with different ticks value
 * Experment with different initial RPM for turning 
 * Tune going straight for 0 to 9 blocks, check if overshooting anypart in blocks in maze
 * 
 * 3.Tuning calibration
 * Experiment with different loop counter
 * Experiment with different motor speed and brake force
 * Experiment with delay period during calibration
 *  
 * Notes & Issues (from previous version)
 * - Robot stray away after a L shape dead end
 * - Robot might be very inaccurate after 5, 6 runs => Voltage drop
 * - Slipping in straight and turn motion
 * =========================================
 * - implement new calibrate logic:
 * Left side calibrate
 * - Improve Calibration of PID => Make sure accurate at different voltage or make sure PID is calibrated at a suitable voltage
...
PID PIDControlStraight(&currentTick1, &speed1, &currentTick2, 2.0 ,0.01 ,0, DIRECT);
...
void move_forward(double distance){
  //at 6.10v to 6.20v
   ...
   target_tick = 310.69*distance - 19.067;
   double tick_travelled = 0;
...
//   Serial.println(speed2);
   ...
   if(FASTEST_PATH){//turn on PID if fastest path
