...
/*
 * v1.2.2 - Based on v1.2.1, the robot suffered from infinite increase in M1_speed_rpm & M2_speed_rpm.
 * I will be using v1.2.2 to debug and make changes to the exisitng code to solve the above mentioned problem.
 * Findings: 
 * 1. setpoint_rpm was set too high at 200, changed to 40. Aiming for 2-4 revolution per sec.
 * 2. sampling time is constantly equals to 1 after settling time.
 * 
 * Changes:
 * Updated KP, KD, KI values
 * Set functions to print values for serial plotters instead of individual serial.print
 * 
 * Tests:
 * Was able to see the PID's effect. Consider running the PID controller at a higher frequency.
 */
...
void PIDController(int directionflag)
{
  ...
  M2_speed_rpm += (E2_error_rpm * KP2) + (E2_prev_error_rpm * KD2); // + (E2_sum_error_rpm * KI1);
