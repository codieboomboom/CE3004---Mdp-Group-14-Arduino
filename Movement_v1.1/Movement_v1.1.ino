...
void PIDController(int directionflag)
{
  ...
  //Calculation
  M1_speed = (E1_error_ticks * (KP * 0.95)) + (E1_prev_error * KD) + (E1_sum_error * KI);
...
//Not complete
void sensor_reading(){
  double distance_FRONT = sensor.getDistance();
}

void front_Calibrate(double front_distance){
  while(sensor.getDistance() != 20){
    if(sensor.getDistance() > 20){
      md.setSpeeds(100, 100);
    }
    else if(sensor.getDistance() < 20){
      md.setSpeeds(-100, -100);
    }
  }
  md.setBrakes(400, 400);
